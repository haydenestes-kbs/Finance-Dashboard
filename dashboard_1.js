// =============================================================
// KBS FP&A DASHBOARD — Ben Fremont · May 2026
// =============================================================
...
const REVIEWER = 'Ben Fremont';                  // shown as comment author
...
// =============================================================
// SOURCE DATA — YTD actuals from the May close (Jan–May 2026)
// Used as the budget baseline for the forecast.
// =============================================================
const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
const ACTUAL_MONTHS = 5;            // Jan–May are actuals
const CURRENT_MONTH_IDX = 4;        // May

// Each line: actuals[Jan..May]. Vendor = the source attribution from the data.
const lineItems = [
  { id:'salary',   label:'Salary',                                cat:'payroll', vendor:'Employees', actuals:[88421,93957,89857,86315,86361] },
  { id:'benefits', label:'Employee Benefits',                     cat:'benefits', vendor:'Employees', actuals:[9441,7543,7348,6885,7068] },
  { id:'anthem',   label:'60100 · Employee Benefits',             cat:'benefits', vendor:'Anthem Blue Cross (KBS)', actuals:[4422,4422,4422,5280,5214] },
  { id:'ccsi',     label:'60203 · Service Center Expenses',       cat:'service', vendor:'Call Center Services Intl (KBS)', actuals:[13043,16095,18759,14580,20651] },
  { id:'accordion',label:'61005 · Prof Fees — Accounting & Audit',cat:'prof', vendor:'Accordion Partners (KBS)', actuals:[0,600,0,0,0] },
  { id:'shankly',  label:'61005 · Prof Fees — Accounting & Audit',cat:'prof', vendor:'Shankly Advisory Partners (KBS)', actuals:[5600,0,0,0,0] },
  { id:'travel',   label:'60301 · Travel',                        cat:'te', vendor:'Employees', actuals:[1234,0,366,0,0] },
  { id:'mileage_e',label:'60302 · Mileage Reimbursement',         cat:'te', vendor:'Employees', actuals:[548,0,151,0,0] },
  { id:'mileage_m',label:'60302 · Mileage Reimbursement',         cat:'te', vendor:'Motus Reimbursement (KBS)', actuals:[700,0,0,0,0] },
  { id:'meals',    label:'60303 · Meals / Ent',                   cat:'te', vendor:'Employees', actuals:[0,549,41,0,0] },
  { id:'parking',  label:'60304 · Parking Expense',               cat:'te', vendor:'Employees', actuals:[0,85,0,0,0] },
  { id:'fed_unemp',label:'55013 · Direct Labor — Fed Unemployment',cat:'other', vendor:'Employees', actuals:[51,51,51,51,51] },
  { id:'hourly',   label:'60002 · Hourly',                        cat:'other', vendor:'Employees', actuals:[5734,5120,-1280,0,0] },
  { id:'overtime', label:'60003 · Overtime',                      cat:'other', vendor:'Employees', actuals:[453,752,-187,0,0] },
  { id:'bonus',    label:'60006 · Indirect — Bonus',              cat:'other', vendor:'Employees', actuals:[0,600,-200,0,0] },
  { id:'vacation', label:'60009 · Indirect — Vacation',           cat:'other', vendor:'Employees', actuals:[-158,-138,0,0,0] },
  { id:'supplies', label:'60503 · Office Supplies',               cat:'other', vendor:'Other', actuals:[0,0,0,0,100] },
];

// Hayden Estes — NON-BUDGETED new hire. Director, Finance Transformation.
// $190,000/yr. Starts May 31, 2026 → first full month is June.
// Benefits estimated at YTD benefits/salary ratio (~13.95%).
const HAYDEN_SALARY_ANNUAL = 190000;
const haydenMonthlySalary = Math.round(HAYDEN_SALARY_ANNUAL / 12);          // 15,833
const benefitsRatio = (function(){
  const sal = lineItems.find(l=>l.id==='salary').actuals.reduce((a,b)=>a+b,0);
  const ben = lineItems.find(l=>l.id==='benefits').actuals.reduce((a,b)=>a+b,0)
            + lineItems.find(l=>l.id==='anthem').actuals.reduce((a,b)=>a+b,0);
  return ben/sal;
})();
const haydenMonthlyBenefit = Math.round(haydenMonthlySalary * benefitsRatio); // ~2,208

// Hayden line items — actuals are 0 (not present in Jan–May), forecast adds salary/benefits Jun–Dec
const haydenLines = [
...
// =============================================================
// STATE & PERSISTENCE  (cloud-first, local fallback)
//   - forecast values (per line, Jun–Dec)
//   - comments (keyed by tab + line + month)
// =============================================================
const LS_FORECAST = 'kbs_fpa_forecast_v1';
const LS_COMMENTS = 'kbs_fpa_comments_v1';
...
function buildDefaultForecast(){
  ...
  haydenLines.forEach(l => {
    // actuals (Jan–May) = 0; forecast Jun–Dec = monthly amount (idx 5..11)
    fc[l.id] = months.map((m,i)=> i<ACTUAL_MONTHS ? 0 : l.monthly);
  });
  return fc;
}

let forecastState = buildDefaultForecast();
let commentsState = {};   // { "forecast|lineId|monthIdx": [ {author, text, ts} ] }

function setConn(ok, label){
  const cls = ok ? '' : 'off';
  document.getElementById('connBadge').innerHTML =
    `<span class="live-dot ${cls}"></span>${label}`;
  document.getElementById('headerBadge').innerHTML =
    `<span class="live-dot ${cls}"></span>${label}`;
  document.getElementById('connBadge').className = 'sync-badge' + (ok?'':' off');
  document.getElementById('headerBadge').className = 'sync-badge' + (ok?'':' off');
}

async function loadAll(){
  if (CLOUD) {
    try {
      // forecast: single row keyed 'ben'
      const { data: fcRow } = await sb.from('forecast').select('data').eq('reviewer','ben').maybeSingle();
      if (fcRow && fcRow.data) forecastState = { ...buildDefaultForecast(), ...fcRow.data };
      const { data: cmts } = await sb.from('comments').select('*').order('created_at',{ascending:true});
      if (cmts) {
        commentsState = {};
        cmts.forEach(c => {
          const k = c.cell_key;
          (commentsState[k] = commentsState[k] || []).push(
      ...
      setConn(false, 'Cloud error · local mode');
    }
  } else {
    setConn(false, 'Local mode · not connected');
  }
  // local fallback
  try {
    const fc = localStorage.getItem(LS_FORECAST);
    if (fc) forecastState = { ...buildDefaultForecast(), ...JSON.parse(fc) };
    const cm = localStorage.getItem(LS_COMMENTS);
    if (cm) commentsState = JSON.parse(cm);
  } catch(e){}
}

async function saveForecastCloud(){
  localStorage.setItem(LS_FORECAST, JSON.stringify(forecastState));
  if (CLOUD) {
    try {
      await sb.from('forecast').upsert({ reviewer:'ben', data:forecastState, updated_at:new Date().toISOString() }, { onConflict:'reviewer' });
    } catch(e){ console.warn('forecast save failed', e); toast('Saved locally — cloud unavailable', true); }
  }
}
...
async function addCommentCloud(cellKey, body){
  ...
  if (CLOUD) {
    ...
    } catch(e){ console.warn('comment save failed', e); toast('Comment saved locally — cloud unavailable', true); }
  }
  (commentsState[cellKey] = commentsState[cellKey] || []).push(entry);
  localStorage.setItem(LS_COMMENTS, JSON.stringify(commentsState));
}

async function deleteCommentCloud(cellKey, idx){
  const list = commentsState[cellKey] || [];
  const entry = list[idx];
  if (CLOUD && entry && entry.id) {
    try { await sb.from('comments').delete().eq('id', entry.id); }
...
function renderTE(){
  ...
  const display = [
    { label:'60301 · Travel', vals: lineItems.find(l=>l.id==='travel').actuals },
    { label:'60302 · Mileage Reimbursement', vals: lineItems.find(l=>l.id==='mileage_e').actuals.map((v,i)=> v + lineItems.find(l=>l.id==='mileage_m').actuals[i]) },
    { label:'60303 · Meals / Ent', vals: lineItems.find(l=>l.id==='meals').actuals },
    { label:'60304 · Parking', vals: lineItems.find(l=>l.id==='parking').actuals },
  ];
  let body='';
  display.forEach(d=>{
...
document.getElementById('saveForecast').addEventListener('click', async ()=>{
  await saveForecastCloud();
  toast(CLOUD ? 'Saved to cloud · Jae will be notified' : 'Saved locally · connect cloud to share');
});
